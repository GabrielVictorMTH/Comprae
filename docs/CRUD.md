...
17. [Passo 14: Integrar Categorias com Outros Módulos](#passo-14-integrar-categorias-com-outros-módulos)
18. [Padrões e Boas Práticas](#padrões-e-boas-práticas)
19. [Troubleshooting](#troubleshooting)
20. [Exercícios Propostos](#exercícios-propostos)
...
## Passo 14: Integrar Categorias com Outros Módulos

### Objetivo

Agora que você implementou o CRUD completo de categorias, é hora de integrá-las com outros módulos do projeto que dependem delas, especialmente o módulo de **Anúncios (Produtos)**.

### Por que esta integração é necessária?

O modelo de Anúncios possui uma **foreign key** para Categoria (`id_categoria`). Para que o sistema funcione completamente, precisamos:

1. Permitir que produtos sejam associados a categorias
2. Exibir o nome da categoria nos produtos
3. Permitir filtrar produtos por categoria
4. Criar dados seed de categorias para testes

---

### 14.1. Restaurar Relacionamento no Modelo de Anúncio

**Arquivo**: `model/anuncio_model.py`

#### Adicionar Import

No início do arquivo, adicione o import da Categoria:
...
from model.categoria_model import Categoria  # ← ADICIONE ESTA LINHA
```

#### Adicionar Atributo de Relacionamento

No final da classe `Anuncio`, adicione o atributo `categoria`:
...
class Anuncio:
    ...
    categoria: Optional[Categoria] = None  # ← ADICIONE ESTA LINHA
```

**Por que fazer isso?**
- `id_categoria`: Armazena apenas o ID (FK no banco)
- `categoria`: Armazena o objeto completo (facilita acesso aos dados como `anuncio.categoria.nome`)

---

### 14.2. Criar Dados Seed de Categorias

**Arquivo**: `util/seed_data.py`

Adicione uma função para criar categorias padrão:

#### 1. Adicionar Imports

```python
from repo import usuario_repo, categoria_repo  # ← Adicione categoria_repo
from model.usuario_model import Usuario
from model.categoria_model import Categoria  # ← ADICIONE ESTA LINHA
...
#### 2. Criar Função de Seed

Adicione esta função **antes** da função `inicializar_dados()`:

```python
def carregar_categorias_seed():
    """
    Carrega categorias padrão do sistema.
    ...
    if len(categorias_existentes) > 0:
        logger.info(f"Já existem {len(categorias_existentes)} categorias cadastradas. Seed não será executado.")
        return

    categorias_seed = [
        Categoria(id=0, nome="Eletrônicos", descricao="Produtos eletrônicos e tecnologia"),
        Categoria(id=0, nome="Livros", descricao="Livros novos e usados de diversos gêneros"),
        Categoria(id=0, nome="Móveis", descricao="Móveis e artigos de decoração"),
        Categoria(id=0, nome="Vestuário", descricao="Roupas, calçados e acessórios"),
        Categoria(id=0, nome="Esportes", descricao="Artigos esportivos e fitness"),
    ]
    ...
    for cat in categorias_seed:
        try:
            ...
            if resultado:
                logger.info(f"✓ Categoria '{cat.nome}' criada com sucesso (ID: {resultado.id})")
                categorias_criadas += 1
            else:
                logger.error(f"✗ Falha ao inserir categoria '{cat.nome}' no banco")
                categorias_com_erro += 1
        except Exception as e:
            logger.error(f"✗ Erro ao processar categoria '{cat.nome}': {e}")
            categorias_com_erro += 1
...
#### 3. Chamar a Função no inicializar_dados()

Modifique a função `inicializar_dados()` para incluir o seed de categorias:
...
def inicializar_dados():
    ...
    try:
        ...
        carregar_categorias_seed()  # ← ADICIONE ESTA LINHA
        logger.info("=" * 50)
...
### 14.3. Atualizar Testes de Anúncios

**Arquivo**: `tests/test_anuncio_repo.py`

Os testes de anúncios precisam de categorias válidas. Modifique a fixture:

#### Trocar Fixture Mockada por Fixture Real

Substitua a fixture `categoria_teste` atual por:

```python
@pytest.fixture
def categoria_teste():
    """Fixture para criar categoria real nos testes"""
    from repo import categoria_repo
    from model.categoria_model import Categoria
    import uuid
    ...
    categoria_inserida = categoria_repo.inserir(categoria)
    return categoria_inserida.id if categoria_inserida else None
```
...
```python
from repo import anuncio_repo, usuario_repo, categoria_repo  # ← Adicione categoria_repo
from model.categoria_model import Categoria  # ← ADICIONE ESTA LINHA
```

#### Restaurar Atributo categoria nos Objetos Anuncio

Em todos os lugares onde você cria objetos `Anuncio` nos testes, adicione:

```python
categoria=None,  # ← ADICIONE esta linha em cada criação de Anuncio
```
...
def test_inserir_anuncio_valido(self, vendedor_teste, categoria_teste):
    anuncio = Anuncio(
        ...
        ativo=True,
        vendedor=None,
        categoria=None  # ← ADICIONE ESTA LINHA
    )
...
**Dica**: Use buscar e substituir (Ctrl+H) para adicionar `,\n        categoria=None` em todos os locais.

---

### 14.4. Atualizar Testes de Pedidos

**Arquivo**: `tests/test_pedido_repo.py`

Faça as mesmas alterações que fez em `test_anuncio_repo.py`:

1. Adicionar imports de `categoria_repo` e `Categoria`
2. Trocar a fixture mockada pela fixture real
3. Adicionar `categoria=None` em todos os objetos Anuncio criados nos testes

---

### 14.5. Testar as Integrações

Após fazer todas as modificações acima, execute:
...
- ✓ Categorias seed criadas (se era primeira execução)

#### 2. Testar CRUD de Produtos

1. Acesse: `http://localhost:8406/admin/produtos/cadastrar`
2. **Verifique**: O campo "Categoria" deve mostrar as 5 categorias seed
3. Crie um produto associando-o a uma categoria
4. **Verifique**: Na listagem de produtos, deve aparecer o nome da categoria

#### 3. Executar Testes Automatizados

```bash
pytest tests/test_anuncio_repo.py -v
pytest tests/test_pedido_repo.py -v
```
...
### 14.6. Funcionalidades Opcionais (Exercícios)

Agora que a integração está completa, você pode adicionar funcionalidades avançadas:

#### 🎯 Exercício 1: Filtrar Produtos por Categoria

**Objetivo**: Adicionar dropdown de filtro na listagem de produtos

**Onde modificar**: `routes/admin_produtos_routes.py` e `templates/admin/produtos/listar.html`

**Dicas**:
- Use `categoria_repo.obter_todos()` para popular o dropdown
- Adicione parâmetro `?categoria_id=X` na query string
- Use `anuncio_repo.obter_por_categoria(id)` para filtrar

#### 🎯 Exercício 2: Exibir Quantidade de Produtos por Categoria

**Objetivo**: Mostrar contador na listagem de categorias

**Onde modificar**: `repo/anuncio_repo.py` (criar função `contar_por_categoria`) e `routes/admin_categorias_routes.py`

**Dica**: Use SQL `SELECT COUNT(*) FROM anuncio WHERE id_categoria = ? AND ativo = 1`

#### 🎯 Exercício 3: Impedir Exclusão de Categoria com Produtos

**Objetivo**: Validar antes de excluir categoria

**Onde modificar**: `routes/admin_categorias_routes.py` na função `post_excluir`

```python
# Verificar se há produtos vinculados
produtos = anuncio_repo.obter_por_categoria(id)
if produtos:
    informar_erro(request, f"Não é possível excluir. Existem {len(produtos)} produto(s) vinculado(s) a esta categoria.")
    return RedirectResponse(...)
```
...
Após completar este passo, você terá:

- ✅ CRUD de Categorias funcionando
- ✅ Produtos associados a categorias
- ✅ Dados seed de categorias
- ✅ Testes passando
- ✅ Sistema totalmente integrado

**Parabéns!** 🎉 Você completou a implementação completa do CRUD de Categorias e sua integração com o restante do sistema!
