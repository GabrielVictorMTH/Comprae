...
class TestTema:
    ...
    def test_get_tema_requer_admin(self, comprador_autenticado):
        """Comprador não deve acessar seletor de temas"""
        response = comprador_autenticado.get("/admin/tema", follow_redirects=False)
    ...
    def test_comprador_nao_pode_aplicar_tema(self, comprador_autenticado):
        """Comprador não deve poder aplicar tema"""
...
class TestAuditoria:
    ...
    def test_get_auditoria_requer_admin(self, comprador_autenticado):
        """Comprador não deve acessar auditoria"""
        response = comprador_autenticado.get("/admin/auditoria", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]
    ...
    def test_comprador_nao_pode_filtrar_logs(self, comprador_autenticado):
        """Comprador não deve poder filtrar logs"""
        data_hoje = agora().strftime("%Y-%m-%d")
