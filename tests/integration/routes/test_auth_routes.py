...
class TestLogin:
    ...
    def test_usuario_logado_nao_acessa_login(self, comprador_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /login"""
        response = comprador_autenticado.get("/login", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
...
class TestCadastro:
    ...
    def test_cadastro_cria_usuario_com_perfil_comprador(self, client):
        """Cadastro público deve criar usuário com perfil COMPRADOR (Enum Perfil)"""
        ...
        assert usuario.perfil == Perfil.COMPRADOR.value  # Usa Enum Perfil


class TestLogout:
    """Testes de logout"""

    def test_logout_limpa_sessao(self, comprador_autenticado):
        """Logout deve limpar sessão e redirecionar para raiz"""
        response = comprador_autenticado.get("/logout", follow_redirects=False)

        # Deve redirecionar para raiz do site
        assert_redirects_to(response, "/")

    def test_logout_desautentica_usuario(self, comprador_autenticado):
        """Após logout, usuário não deve ter acesso a áreas protegidas"""
        # Fazer logout
        comprador_autenticado.get("/logout")

        # Tentar acessar área protegida
        response = comprador_autenticado.get("/chamados/listar", follow_redirects=False)
...
class TestAutorizacao:
    ...
    def test_usuario_autenticado_acessa_area_protegida(self, comprador_autenticado):
        """Usuário autenticado deve acessar áreas protegidas"""
        response = comprador_autenticado.get("/chamados/listar")
        assert response.status_code == status.HTTP_200_OK

    def test_comprador_nao_acessa_area_admin(self, comprador_autenticado):
        """Comprador não deve acessar áreas administrativas"""
...
class TestCadastroAdicional:
    ...
    def test_usuario_logado_nao_acessa_cadastro(self, comprador_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /cadastrar"""
        response = comprador_autenticado.get("/cadastrar", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
